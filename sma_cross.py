...
class SMACrossStrategy(bt.SignalStrategy):
    ...
    def next(self):
        ...
        if not self.position:
            if self.sma_small[0] > self.sma_big[0] and self.sma_small[-1] < self.sma_big[-1]:
                self.order = self.buy()
        else:
            if self.sma_small[0] < self.sma_big[0] and \
                self.sma_small[-1] > self.sma_big[-1]:
...
strats = cerebro.optstrategy(
        SMACrossStrategy,
        maperiod_small=range(2, 100),
        maperiod_big=range(10, 1000),
)
